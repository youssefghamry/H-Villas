                ...
                if ( ! empty($separator) ) {
                    ...
                    if ( $separator == 'zigzag' ) {
                        $element_html .='<div class="separator-zigzag ' . $align . '">
                                            <figure><img src="' . trailingslashit(get_template_directory_uri()) . 'assets/images/zigzag-img1.png" alt="img"/></figure>
                                        </div>';
                    }
                }
                $element_html .= '</div>';
        ...
        function wp_rem_clients() {
            ...
            if ( ! empty($clients_images) ) {
                ...
                foreach ( $clients_images as $key => $image ) {
                    ...
                    $html .= '<li><figure><a href="' . esc_url($url) . '"><img src="' . esc_url($image) . '" alt="img" title="' . esc_html($title) . '"></a></figure></li>';
                }
    ...
    if ( ! function_exists('wp_rem_default_pagination') ) {
        ...
        function wp_rem_default_pagination() {
            ...
            if ( $links ) :
                ...
                <?php echo wp_rem_cs_allow_special_char($links); ?>
                </nav><!-- navigation -->
                <?php
            endif;
        }
    ...
    if ( ! function_exists('wp_rem_cs_custom_format_gallery') ) {
        ...
        function wp_rem_cs_custom_format_gallery($string, $attr) {
            ...
            if ( isset($attr['ids']) ) {
                ...
                $posts = get_posts(array( 'inc'.'lude' => $attr['ids'], 'post_type' => 'attachment' ));
                foreach ( $posts as $imagePost ) {
                    $output .=' <div class="' . $col_class . '"> <div class="media-holder"> <figure> <img src="' . wp_get_attachment_image_src($imagePost->ID, 'wp_rem_cs_media_3')[0] . '" alt="img"> </figure> </div> </div> ';
                }
                $output .= " </div> ";
            }
            return $output;
    ...
    if ( ! function_exists('wp_rem_cs_var_get_pagination') ) {
        ...
        function wp_rem_cs_var_get_pagination($total_pages = 1, $page = 1, $shortcode_paging = '') {
            ...
            if(function_exists('homevillas_global_session')){
                $QUERY_STRING = homevillas_global_session('QUERY_STRING');
            }
            ...
            echo wp_rem_cs_allow_special_char($wp_rem_cs_var_pages);
        }
...
//===============================  This function controlling excerpt on front end	======================
    ...
    if ( ! function_exists('wp_rem_cs_social_network') ) {
        ...
        function wp_rem_cs_social_network($header9, $icon_type = '', $tooltip = '', $ul_class = '', $no_title = true) {
            ...
            if ( isset($tooltip) && $tooltip <> '' ) {
             ...
             if ( (is_object($wp_rem_cs_var_options['wp_rem_cs_var_social_net_url']) || is_array($wp_rem_cs_var_options['wp_rem_cs_var_social_net_url'])) && isset($wp_rem_cs_var_options['wp_rem_cs_var_social_net_url']) and count($wp_rem_cs_var_options['wp_rem_cs_var_social_net_url']) > 0 ) {
                $i = 0;
                ...
                if ( is_array($wp_rem_cs_var_options['wp_rem_cs_var_social_net_url']) ):
                    foreach ( $wp_rem_cs_var_options['wp_rem_cs_var_social_net_url'] as $val ) {
                        if ( '' !== $val ) {
                            ...
                            $html .= '<li>';
                            $html .= '<a href="' . $val . '" data-original-title="' . $data_original_title . '" data-placement="top" ' . wp_rem_cs_allow_special_char($tooltip_data) . ' class="colrhover"  target="_blank">';
                            if (isset($wp_rem_cs_var_options['wp_rem_cs_var_social_net_awesome'][$i]) && $wp_rem_cs_var_options['wp_rem_cs_var_social_net_awesome'][$i] <> '' && isset($wp_rem_cs_var_options['wp_rem_cs_var_social_net_awesome'][$i]) ) {
    ...
    if ( ! function_exists('wp_rem_cs_write_stylesheet_content') ) {
        ...
        function wp_rem_cs_write_stylesheet_content() {
            ...
            $newStr = $fileStr; // preg_replace(array_keys($regex), $regex, $fileStr);
            ...
            if ( is_home() || is_front_page() ) {
                ...
                if ( is_category() ) {
                    ...
                    if ( function_exists("is_shop") && get_post_type() == 'product' ) {
                        $wp_rem_cs_shop_page_id = wc_get_page_id('shop');
    ...
    if ( ! function_exists('wp_rem_post_views_count') ) {
        ...
        function wp_rem_post_views_count($postID) {
            ...
            $cs_views_counter = $cs_views_counter <> '' ? $cs_views_counter : 0;
